...
% In this work we'll represent several views about the languages, such as the popularity of a language, the most used languages, the evolution of a language over the years or even over the various rounds in the same year.
...
Several scripts were implemented to do the web scrapping from the \textbf{go-hero.net} website, that is, to retrieve the data that will be stored and later analyzed% , where each one of the scripts retrieved a specific table of information from the website.
...
% and stores that data in the \textbf{allLanguagesUsed.txt} file.
...
% the dominance of \textit{C++} is even bigger in the last and most important rounds, passing from around 50\% in the first 4 rounds to more than 70\% in the 3 final rounds.
...
% isso deve-se a estar representado por regioes autonomas
